...
function Generator(options, loc, toUpper) {
  var json = {};
  (typeof toUpper == 'boolean' && toUpper) ? toUpper = true : toUpper = false;
  this.errors = {};
  this.json = json;
...
Generator.prototype.capitalize = function(str,toUpper) {
  if(!toUpper) return str[0].toUpperCase() + str.substring(1).toLowerCase();
  return str.toUpperCase();
};

Generator.prototype.getName = function(name, k, toUpper) {
  name = name || '';
  if(!toUpper) return name+ this.capitalize(k);
  return (name&&name!=='') ? name+"_"+ this.capitalize(k,toUpper) : name+this.capitalize(k,toUpper);
};
